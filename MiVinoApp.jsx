...
{ready && (
<button onClick={handleCapture} style={{ width: ‘72px’, height: ‘72px’, borderRadius: ‘50%’, background: ‘#fff’, border: ‘4px solid rgba(255,255,255,0.4)’, cursor: ‘pointer’, display: ‘flex’, alignItems: ‘center’, justifyContent: ‘center’ }}>
...
<button onClick={() => setEditingType(isEditingThisType ? null : wine.id)} style={{ background: wineType.bg, border: `1px solid ${wineType.border}`, color: wineType.color, fontSize: ‘10px’, padding: ‘2px 8px’, borderRadius: ‘20px’, cursor: ‘pointer’ }}>
{wineType.label}
</button>
</div>
{isEditingThisType && (
...
<button key={key} onClick={() => handleChangeType(wine.id, key)} style={{ background: t.bg, border: `1px solid ${t.border}`, color: t.color, fontSize: ‘11px’, padding: ‘4px 10px’, borderRadius: ‘20px’, cursor: ‘pointer’, fontWeight: wine.wineType === key ? ‘600’ : ‘400’ }}>
{t.label}
</button>
))}
</div>
